...
window.Webflow.push(() => {
  ...
  function textFX() {
    ...
    $(block).on('mouseover', function () {
      if (
        Number($(this).find('.blog-title').first().width()) >
        Number($('.blog-content-item').width())
      ) {
  ...
  const count = Number(window.location.search.slice(-1));

  $('#next-page-btn').on('click', function () {
    if (window.location.search === '') {
      window.location.search = 'ff8076c3_page=2';
      window.location.hash = '#blog-header';
    }
    window.location.search = `ff8076c3_page=${count + 1}`;
  });
